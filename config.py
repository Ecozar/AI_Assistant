...
MEMORY_SETTINGS = {
    ...
    'auto_tag': {
        ...
        'default_threshold': 0.125,  # Add default threshold
    }
}
...
def get_ui_settings():
    """Get settings from ui_settings.json"""
    try:
        config_path = os.path.join(PROJECT_ROOT, "config", "ui_settings.json")
        with open(config_path, 'r') as f:
            return json.load(f)
    except Exception as e:
        logging.error(f"Error loading UI settings: {e}")
        return {}  # Let UI handle defaults
