...
class MemoryManager:
    ...
    # From memory_decay.py
    def _start_decay_service(self):
    ...
    def _decay_loop(self):
        """Background thread for applying decay"""
        while not self._stop_decay:
            try:
                # Get all memory IDs that need decay
                with db_manager.get_cursor() as cursor:
                    cursor.execute("""
                        SELECT id FROM memory_items 
                        WHERE layer != 'permanent'
                    """)
                    memory_ids = [row['id'] for row in cursor.fetchall()]
                    
                # Apply decay to each memory individually
                for memory_id in memory_ids:
                    self._apply_decay(memory_id)
                    
                time.sleep(MEMORY_SETTINGS['decay']['check_interval'])
            except Exception as e:
                logging.error(f"Error in decay loop: {e}")
                time.sleep(60)  # Wait before retrying
    
    def _apply_decay(self, memory_id: str = None):
        """Apply decay based on layer and time"""
        try:
            with db_manager.get_cursor() as cursor:
                if memory_id:
                    # Single memory decay
                    cursor.execute("SELECT layer, importance FROM memory_items WHERE id = ?", 
                                 (memory_id,))
                    memory = cursor.fetchone()
                    if not memory:
                        logging.warning(f"Memory {memory_id} not found for decay")
                        return
                    
                    # Get decay rate for layer
                    layer = memory['layer']
                    decay_rate = MEMORY_SETTINGS['decay']['layers'][layer]['rate']
                    min_importance = MEMORY_SETTINGS['decay']['layers'][layer]['min_importance']
                    ...
                    # Bulk decay
                    cursor.execute("SELECT COUNT(*) as count FROM memory_items WHERE layer != 'permanent'")
                    if cursor.fetchone()['count'] == 0:
                        logger.warning("No memories to decay")
                        return

                    cursor.execute("""
                        UPDATE memory_items
                        SET importance = MAX(0, importance - (? * decay_factor)),
                            decay_factor = decay_factor * 1.1
                        WHERE layer != 'permanent'
                    """, (MEMORY_SETTINGS['decay']['rate'],))
                
                # Verify changes
                cursor.execute("SELECT changes() as count")
                if cursor.fetchone()['count'] == 0:
                    logger.warning("No memories were decayed")
                
        except Exception as e:
            logging.error(f"Error applying decay: {e}")

    def store_memory(self, content: str, topics: Optional[Set[str]] = None) -> str:
        """Store memory with auto-tagging if topics not provided"""
        try:
            if topics is None:
                # Use auto_tagger as SSOT for tag suggestions
                suggestions = auto_tagger.get_suggested_tags(content)
                topics = {tag for tag, _ in suggestions}
                logger.debug(f"Auto-generated topics: {topics}")
            ...
            logger.debug(f"Stored memory {memory_id} with topics {topics}")
            return memory_id
    ...
    def _row_to_memory_item(self, row: sqlite3.Row) -> MemoryItem:
        ...
        return MemoryItem(
            ...
            related_items=set(json.loads(row['related_items']))
        ) 
